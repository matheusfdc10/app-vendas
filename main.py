...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            requisicao_dic = requisicao.json()

            # preencher lista de foto de perfil
    ...
    def mudar_foto_perfil(self, foto,  *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            mensagem_texto.text = "Usuário não encontrato"
        else:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                data=info)
    ...
    def  selecionar_cliente(self, foto, *args):
        ...
        for item in list(lista_clientes.children):
            item.color = (1, 1, 1, 1)
            # pintar de azul as letras do item que selecionamos
            try:
                texto = item.text
    ...
    def adicionar_venda(self):
        ...
        # dado que ja preencheu tudo, vamos executa o codigo de adicionar venda   
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            requests.post(f"https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                            data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendashash-37aef-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()

        # preencher lista de foto da empresa
        foto_perfil = self.root.ids["foto_perfil"]
